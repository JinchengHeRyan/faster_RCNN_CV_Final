...
def vis_bbox(img, bbox, label=None, score=None, ax=None):
    ...
    for i, bb in enumerate(bbox):
        ...
        width = bb[3] - bb[1]
        ax.add_patch(
            plot.Rectangle(xy, width, height, fill=False, edgecolor="red", linewidth=2)
        )
...
class Visualizer(object):
    ...
    def __init__(self, env="default", **kwargs):
        self.vis = visdom.Visdom(
            "localhost", env=env, use_incoming_socket=False, **kwargs
        )
        self._vis_kw = kwargs
    ...
    def img(self, name, img_, **kwargs):
        ...
        """
        self.vis.images(
            t.Tensor(img_).cpu().numpy(), win=name, opts=dict(title=name), **kwargs
        )
